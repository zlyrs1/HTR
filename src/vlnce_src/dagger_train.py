...
def initialize_trainer(dagger_it=0, checkpoint_folder=None):
    ...
    if dagger_it == 0:
        ...
        checkpoint_path = poll_checkpoint_folder(str(checkpoint_folder), int(dagger_it * args.epochs - 2))
...
def _eval_checkpoint(checkpoint_path: str, writer, tok, checkpoint_index: int = 0) -> None:
...
if __name__ == "__main__":
    ...
    if args.run_type == 'train':
        ...
        for dagger_it in range(int(args.dagger_it)):
            # 断点续训，将之前时间戳的模型文件拷贝到本次运行对应的时间戳文件夹下
            ...
            # 派生（spawn）出 world_size个新子进程执行训练
            world_size = torch.cuda.device_count()
            mp.spawn(train_vlnce, args=(world_size, dagger_it, config), nprocs=world_size)
