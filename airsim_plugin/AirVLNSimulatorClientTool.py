...
class AirVLNSimulatorClientTool:
    ...
    def run_call(self, airsim_timeout: int=60) -> None:
        # 创建仿真客户端(目前只有一个)，之间通过事件消息处理场景相关的任务
        socket_clients = []
        for index, item in enumerate(self.machines_info):
            socket_clients.append(msgpackrpc.Client(msgpackrpc.Address(item['MACHINE_IP'],
                                                                       item['SOCKET_PORT']), timeout=180))

        # 确保主机与客户端能建立连接，看是否能ping通
        for socket_client in socket_clients:
        ...
        before = time.time()
        # 疑问：这里的关闭链接纯属多余，因为此时self.airsim_clients刚初始化，值都为None
        ...
        def _run_command(index, socket_client: msgpackrpc.Client):
            ...
            ip = result[1][0]        # 主机端返回的IP地址
            ports = result[1][1]     # 主机端返回的batch所有样本对应的airsim端口号
            print("主机端返回的airsim客户端对应端口号:{}".format(ports))

            # 检查主机端返回的ip和端口是否正确
            # assert ip.decode('utf-8') == socket_client.address._host, '打开场景失败'
    ...
    def getImageResponses(self, get_rgb=True, get_depth=True):
            ...
            if scen_id in [1, 7]:
                ...
                while True:
                    try:
                        ImageRequest = []
                        if get_rgb:
                            ImageRequest.append(airsim.ImageRequest("front_0", airsim.ImageType.Scene,
                                                                    pixels_as_float=False, compress=False))
                        if get_depth:
                            ImageRequest.append(airsim.ImageRequest("front_0", airsim.ImageType.DepthVis,
                    ...
                    try:
                        ...
                        if get_rgb:
                            ImageRequest.append(airsim.ImageRequest("front_0", airsim.ImageType.Scene,
                                                                    pixels_as_float=False, compress=False))
                        if get_depth:
                            ImageRequest.append(airsim.ImageRequest("front_0", airsim.ImageType.DepthPerspective,
        ...
        for index_1, _ in enumerate(threads):
            responses.append([])
            for index_2, _ in enumerate(threads[index_1]):
                responses[index_1].append(threads[index_1][index_2].get_result())
    ...
    def setPoses(self, poses: list) -> bool:
        def _setPoses(airsim_client: airsim.VehicleClient, pose: airsim.Pose) -> None:
            ...
            airsim_client.simSetVehiclePose(pose=pose,
