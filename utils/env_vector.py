...
class VectorEnvUtil:
    ...
    def get_obs(self, obs_states) -> Tuple[List[Any], List[Any]]:
        ...
        for index in range(len(obs_states)):
            ...
            self._connection_write_fns[index]((COMMAND_GET_OBS, (index, state)))

        results = [self._connection_read_fns[index]() for index in range(len(obs_states))]
        ...
        for index in range(len(obs_states)):
            ...
            obs.append(self._format_obs_at(index, teacher_action, done, progress))
    ...
    def _format_obs_at(self, index: int, teacher_action, done, progress):
        ...
        # 1
        observations = {
            "instruction": item['instructionCLIP'],
